...
.PARAMETER psDeploymentShare
    ...
    e.g. if the deployment folder is F:\MDTDeployments\PSDBui<#
.SYNOPSIS
.DESCRIPTION
.PARAMETER psDeploymentFolder
    Specify the target MDT/PSD deployment share's physical folder location in the file system.
...
.NOTES
          ...
          Version - 0.0.7 - () - Internationalization fix: replaced hardcoded English account names
                                 (Administrators, Users, SYSTEM, EVERYONE, CREATOR OWNER) with well-known
                                 SIDs resolved to their localized names. The installer now works on
                                 non-English Windows Server (e.g. French: Administrateurs, Utilisateurs,
                                 Tout le monde, CREATEUR PROPRIETAIRE). Also fixed ADK WinPE add-on
                                 version check (was parsing the ADK version instead of the WinPE add-on).
          TODO:
...
$Script:DeploymentToolkitVersion = "0.2.3.7"
...
function Get-PSDLocalizedAccountName {
    <#
    .SYNOPSIS
        Resolves a well-known SID to its localized account name.
    .DESCRIPTION
        Built-in account and group names are localized on non-English versions of Windows
        (e.g. on a French Windows Server: Administrators = Administrateurs, Users = Utilisateurs,
        Everyone = Tout le monde, SYSTEM = AUTORITE NT\Systeme, CREATOR OWNER = CREATEUR PROPRIETAIRE).
        Hardcoding the English names breaks New-SmbShare / Grant-SmbShareAccess / icacls on those systems.
        Well-known SIDs are language-independent, so we translate them at runtime.
    #>
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $true)]
    ...
    try {
        ...
        Write-PSDInstallLog -Message "Unable to translate well-known SID $Sid to an account name: $($_.Exception.Message)" -LogLevel 3
        exit
    }
}

# Well-known SIDs (language-independent identifiers)
$script:SidAdministrators = 'S-1-5-32-544'  # BUILTIN\Administrators / BUILTIN\Administrateurs
$script:SidUsers          = 'S-1-5-32-545'  # BUILTIN\Users / BUILTIN\Utilisateurs
$script:SidSystem         = 'S-1-5-18'      # NT AUTHORITY\SYSTEM / AUTORITE NT\Systeme
$script:SidEveryone       = 'S-1-1-0'       # Everyone / Tout le monde
$script:SidCreatorOwner   = 'S-1-3-0'       # CREATOR OWNER / CREATEUR PROPRIETAIRE

# Start logging
Set-PSDDefaultLogPath
Write-PSDInstallLog -Message "Starting installation script for $DeploymentToolkitVersion"

# Resolve localized names of the built-in accounts used by this installer.
# This makes the installer compatible with any Windows display language (English, French, German, ...).
$script:AdministratorsAccount = Get-PSDLocalizedAccountName -Sid $script:SidAdministrators
$script:EveryoneAccount       = Get-PSDLocalizedAccountName -Sid $script:SidEveryone
$script:CreatorOwnerAccount   = Get-PSDLocalizedAccountName -Sid $script:SidCreatorOwner
Write-PSDInstallLog -Message "Localized built-in accounts resolved: Administrators='$($script:AdministratorsAccount)', Everyone='$($script:EveryoneAccount)', Creator Owner='$($script:CreatorOwnerAccount)'"
...
# ADK1809 build:17763
# Fixed: the original script parsed $mdtADK (ADK) instead of $mdtADKPE (WinPE add-on),
# and would throw on a null value. Guard first, then parse the correct variable.
if ($null -eq $mdtADKPE) {
    Write-PSDInstallLog -Message "ADKPE not detected/installed! Aborting." -LogLevel 3 -writetoscreen $true
    exit
}
$ADKPEbuild = [int]([string]$mdtADKPE.Split('.')[2])
if ($ADKPEbuild -lt $script:MinADKVersion) {
    Write-PSDInstallLog -Message "ADKPE not detected/installed! Aborting." -LogLevel 3 -writetoscreen $true
    exit
} else {
...
if (-not ($Upgrade.IsPresent)){
    ...
    $Null = New-SmbShare -Name $psDeploymentShare -Path $psDeploymentFolder -FullAccess $script:AdministratorsAccount -Description $description -ErrorAction Stop
...
if (!($Upgrade)) {
    ...
    # icacls accepts well-known SIDs with the *SID syntax, which is fully language-independent
    # (avoids any localized name / console code page issue, e.g. accented French names).
    icacls $psDeploymentFolder /grant "*$($script:SidUsers):(OI)(CI)(RX)" | Out-Null
    icacls $psDeploymentFolder /grant "*$($script:SidAdministrators):(OI)(CI)(F)" | Out-Null
    icacls $psDeploymentFolder /grant "*$($script:SidSystem):(OI)(CI)(F)" | Out-Null
    Grant-SmbShareAccess -Name $psDeploymentShare -AccountName $script:EveryoneAccount -AccessRight Change -Force | Out-Null
    Revoke-SmbShareAccess -Name $psDeploymentShare -AccountName $script:CreatorOwnerAccount -Force | Out-Null
...
Write-Verbose -Message "PSD is installed to $psDeploymentShare" -Verboseld on a machine called MDTServer  the share will become \\MDTServer\PSDBuild$ 
    This parameter must have a value for an upgrade process.
.PARAMETER Upgrade
    Use this switch to upgrade an existing deployment share.
    In order to use this switch, upgrade an existing deployment share, the parameters psDeploymentFolder and psDeploymentShare must have valid values.
